...
def create_fraud_gauge(probability):
    """Create a modest radial progress circle with gradient"""
    ...
    fig.add_annotation(
        ...
        x=0.5, y=0.52,
        font=dict(size=72, color=color, family="Roboto"),
        showarrow=False
    )
...
def create_feature_importance_chart(input_data):
    """Create a bar chart showing feature importance from the model"""
    feature_names = [
        "Merchant", "Category", "Amount", "Job", "Age", "Hour", "Day", 
        "Month", "Gender", "State", "City Pop", "Trans Lat", "Trans Long", 
        "Merch Lat", "Merch Long"
    ]
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        # Slate blue gradient - neutral, pairs with red/black
        custom_colorscale = [
            [0, '#8e99b3'],      # Light slate blue-gray
            [0.5, '#5c6b8a'],    # Medium slate blue-gray
            [1, '#3d4d66']       # Dark slate blue-gray
        ]
...
with col3:
    age = st.number_input("Age", min_value=0.0, help="Cardholder's age in years")

col1, col2, col3 = st.columns(3, gap="medium")
with col1:
    gender = st.selectbox("Gender", ["Male", "Female"], help="Cardholder's gender")
with col2:
    city_pop = st.number_input(
        "City Population", min_value=0.0, help="Population of the cardholder's city"
    )
with col3:
    lat = st.number_input(
...
with col3:
    merch_long = st.number_input(
        "Merchant Longitude", format="%.4f", help="Longitude of merchant's location"
    )
...
with col2:
    day = st.slider(
        "Transaction Day", 1, 31, 15, help="Day of month when transaction occurred"
    )
with col3:
    month = st.slider(
        "Transaction Month", 1, 12, 6, help="Month of year when transaction occurred"
    )
...
if st.session_state.prediction_type is not None:
    st.markdown(
        "<hr style='margin-top: 0.75rem; margin-bottom: 1.25rem;'>", unsafe_allow_html=True
    )
    ...
    with col1:
        st.plotly_chart(create_fraud_gauge(st.session_state.prediction_proba), use_container_width=True)
        ...
        if st.session_state.prediction_proba >= 0.5:
            st.markdown(
                ...
                <div style='background-color: rgba(255, 75, 75, 0.1); padding: 15px; border-radius: 5px; border-left: 5px solid #ff4b4b; text-align: center;'>
                    <span style='color: #ff4b4b; font-weight: 600; font-size: 1.1em;'>Fraudulent Transaction Detected</span>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if st.session_state.prediction_type in ["fraud", "legit"]:
        st.markdown(
            "<hr style='margin-top: 1rem; margin-bottom: 0.5rem;'>", unsafe_allow_html=True
        )
        case_type = (
            "Fraud Sample"
        ...
        st.subheader(f"{case_type} Input Data")
